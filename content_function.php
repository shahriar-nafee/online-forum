<?php
	...
	$userid="";
	$result1 = mysqli_query($con, "SELECT user_id FROM users WHERE username = '".$username."'");
	while ($row = mysqli_fetch_assoc($result1)) {
		...
		$result3 = mysqli_query($con, "SELECT * FROM course WHERE course_id = '".$scid."' AND student_id = '".$userid."'");
	...
	if (mysqli_num_rows($result3) != 0) {
		...
		if (mysqli_num_rows($select) != 0) {
			...
			echo "<tr><th>Title</th><th>Posted By</th><th>Date Posted</th><th>Views</th><th>Option</th></tr>";
			while ($row = mysqli_fetch_assoc($select)) {
				...
				echo "<tr><td><a href='/forum-tutorial/readtopic/".$cid."/".$scid."/".$row['topic_id']."'>
					 ...
					 <td>";
						  ...
						  {
					       echo" <a href='/forum-tutorial/deletetopic.php?c=".$cid."&s=".$scid."&tid=".$row['topic_id']."'>DELETE</a>";
		...
		header("Location: /forum-tutorial/index.php?status=course-fail");
	...
	function dispreplies($cid, $scid, $tid) {
		include ('dbconn.php');
		$username = $_SESSION['username'];
		$select = mysqli_query($con, "SELECT replies.reply_id,replies.author, comment, replies.date_posted FROM categories, subcategories, 
		...
		if (mysqli_num_rows($select) != 0) {
			...
			while ($row = mysqli_fetch_assoc($select)) {
				echo "<tr><th width='15%'>".$row['author']."</th><td>".$row['date_posted']."<br>".$row['comment']."\n\n</td><td>";

                     
                     
                           if  ($username == $row['author'])
						  {
					       echo "<a href='/forum-tutorial/deletereplies.php?c=".$cid."&s=".$scid."&t=".$tid."&rid=".$row['reply_id']."'>DELETE</a>";
